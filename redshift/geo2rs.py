    ...
    with fiona.open(file_name, "r") as source:
        ...
        with open(output_file, "w") as file:
            ...
            for f in source:
                try:
                    if firstRow:
                        writer.writerow(["geom"] + list(f["properties"].keys()))
                        firstRow = False
                    if epsg != -1:
                        writer.writerow(
...
def execute_redshift_statement(cluster_identifier, database, secret_arn, sql):
    ...
    try:
        ...
        result_status_response = client.describe_statement(Id=query_response["Id"])
        while (
            result_status_response["Status"] != "FAILED"
            and result_status_response["Status"] != "FINISHED"
        ):
            result_status_response = client.describe_statement(Id=query_response["Id"])
        ...
        if result_status_response["Status"] == "FINISHED":
            if result_status_response["HasResultSet"]:
                result_response = client.get_statement_result(Id=query_response["Id"])
                result = result_response["Records"]
                while result_response["NextToken"] != "":
                    result_response = client.get_statement_result(
                        Id=query_response, NextToken=result_response["NextToken"]
                    )
...
def import_file_redshift(
    ...
    try:
        ...
        result = execute_redshift_statement(
            ...
            ).format(table_name, csv_file_path, redshift_role_arn),
        )
...
def get_secret():
    ...
    secret_name = "redshift-bdnb-secret"
...
if __name__ == "__main__":
    input_file = "bnb_export_95.gpkg"
    bucket = "bucketcarto"
    cluster_identifier = "redshift-cluster-1"
    database = "dev"
    redshift_role_arn = "arn:aws:iam::409727658222:role/service-role/AmazonRedshift-CommandsAccessRole-20221105T161828"
    secret_arn = "arn:aws:secretsmanager:eu-west-3:409727658222:secret:redshift-bdnb-awsuser-secret-rmg7jz"
    table_name = "BDNB95"
